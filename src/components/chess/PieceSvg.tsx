...
/**
 * Classic "CBurnett" chess pieces (same family as Lichess default / Wikipedia).
 * Distinct silhouettes — especially the knight profile with mane and eye.
 */
...
export function PieceSvg({ type, color, className, templar }: PieceSvgProps) {
  const key = `${color}${type}` as const;
    ...
    <svg
      ...
      style={
        templar && type === "k"
          ? {
              filter:
                "drop-shadow(0 0 5px color-mix(in oklab, var(--color-templar) 75%, transparent))",
            }
          : undefined
      }
    >
      {PIECES[key]}
      {templar && type === "k" && (
        <circle cx="22.5" cy="4.5" r="2.2" fill="var(--color-templar)" />
      )}
    </svg>
  );
}
...
const PIECES: Record<`${Color}${PieceSymbol}`, ReactNode> = {
  wk: (
    <g {...gCommon}>
